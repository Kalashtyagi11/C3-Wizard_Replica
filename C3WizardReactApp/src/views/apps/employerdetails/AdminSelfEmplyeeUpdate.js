...
// import { getAllCategory, getAllCountry } from '../../../../store/apps/auth/AuthSlice';

const AdminSelfEmplyeeUpdate = () => {
  ...
  const selfEmployeeid = state?.selfEmployeeid;
  ...
  //     (role) => role.title.toUpperCase() === 'PERSONAL DETAILS',
  //   );
  ...
  const parseCustomDate = (value) => {
    ...
    // Case: only digits → e.g. 12122014
    if (/^\d{8}$/.test(value)) {
      const day = parseInt(value.slice(0, 2), 10);
      const month = parseInt(value.slice(2, 4), 10) - 1;
      const year = parseInt(value.slice(4), 10);
      const date = new Date(year, month, day);
      return Number.isNaN(date.getTime()) ? null : date; // ✅ fixed
    }

    // Case: d-m-yyyy or d-MMM-yyyy
    const regex = /^(\d{1,2})[-/](\w+)[-/](\d{4})$/i;
    const match = value.match(regex);
    if (!match) return null;

    let [, day, year] = match;
    const month = match[2]; // keep as const, since not reassigned
  ...
  const handleSubmit = () => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      dispatch(updatePersonal(updatedFormData))
        .unwrap()
        .then((result) => {
          navigate('/admin/self-employed-details');
        })
        .catch((err) => {
  ...
  // const dateTimeNew = new Date(formData?.birthDate)?.toISOString()?.split('T')[0];
  // const dateTimeNew = formData?.birthDate ? new Date(formData.birthDate)?.toISOString()?.split('T')[0] : null;
        ...
        <div className="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
          <ul className="d-flex align-items-center gap-2 mt-3 list-unstyled">
            <li className="fw-medium">
              <Link
                to="/admin/self-employed-details"
                      ...
                      <div className="card-body">
                        ...
                        <div className="row mt-3">
                          <div className="'col-md-6 col-lg-6 col-xl-6">
                            ...
                            <Link to="/admin/self-employed-details">
...
export default AdminSelfEmplyeeUpdate;
